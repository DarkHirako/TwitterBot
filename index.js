...
function getCallback (err,data,response){

    for(var i = 0; i < data.length; i++){
        if(data[i].entities.media != undefined){ //Media exists
            for(var j = 0; j < data[i].entities.media.length; j++){
                var tweetUser = data[i].user.screen_name;
                var imageUrl = data[i].entities.media[j].media_url;
                var imageId = data[i].entities.media[j].id;
